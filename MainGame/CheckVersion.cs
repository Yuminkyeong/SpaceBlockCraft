...
public class CheckVersion : MonoBehaviour
{
    ...
    public static CheckVersion GetInstance()
    {
        if (!instance)
        {
            instance = GameObject.FindObjectOfType<CheckVersion>();
            if (!instance)
                Debug.LogWarning("There needs to be one active CheckVersion script on a VersionCheckManager in your scene.");
        }
        return instance;
    }
