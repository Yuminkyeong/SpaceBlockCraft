...
public class DragSnapper : UIBehaviour, IEndDragHandler, IBeginDragHandler
{
    ...
    public void SelectSnap(float endvalue)
    {
        ...
        StartCoroutine(SnapRectEnd(nextStartNormal,endvalue));
    }
    ...
    private IEnumerator SnapRectEnd(float startNormal, float endNormal)
    {
        if (scrollRect == null)
            throw new System.Exception("Scroll Rect can not be null");
        if (itemCount == 0)
